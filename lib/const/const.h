#ifndef const_h
#define const_h
const int number = 8;
const int dts = 2;
const int dt = 25;
const int dt_telem = 300;
const int dt_line = 30;
const int dt_ultrasonic = 50;
const int max_pwm = 150;
const int base_pwm   = 40;
const int wall_pwm = 40;
const int forward_pwm = 30;
const int roundabout_pwm = 30;
const int correction_motor = 0;
const int threshold_lateral = 400;
const int threshold_calibrated = 500;
const int turn_outside_pwm = 35;
const int turn_inside_pwm = 35;
const int wall_outside_pwm = 80;
const int wall_inside_pwm = 0;
const int wall_setpoint = 6;
const int min_number_sensors_read = 6;
const int stability_delay = 1;
const int chicane_forward_time = 50;
#endif